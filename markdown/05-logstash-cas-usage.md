...
Création d'un hash unique par document, Elastic crée un hash aléatoire, si on rejoue une ingestion pas de doublon avec fingerprint :
...
# Cas pratique guidé (films.csv) - objectif et préparation

Objectif :
- partir d'un petit CSV lisible
- voir exactement ce que Logstash produit (`rubydebug`)
- indexer ensuite dans Elasticsearch
...
```bash
cp films.csv logs/films.csv
```

Dans le conteneur, le chemin devient :
`/usr/share/logstash/logs/films.csv`
...
```text
/usr/share/logstash/logs/tmdb_movies.csv
```
...
# Cas pratique (TMDB) - étape 1 pipeline minimal

```conf
input {
  file {
    path => "/usr/share/logstash/logs/tmdb_movies.csv"
    start_position => "beginning"
